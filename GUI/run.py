...
class ProcessusOpenFile(Processus):
    ...
    def tempdef(self):
        globalparams.data['frames'] = self.parent.parent.videoFrame.open_file()
        globalparams.data['size'] = globalparams.data['frames'][0].shape[:2]
...
class ProcessusDetection(Processus):
    ...
    @staticmethod
    def start():
        globalparams.data['detections'] = detect(globalparams.data['frames'])
